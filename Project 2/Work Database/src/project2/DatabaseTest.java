...
class DatabaseTest {
    ...
    void testDeleteEmployee() throws SQLException {
        ...
        try (Statement stc = d2.sqda.getConnection().createStatement()) {
            ...
            // stc.getConnection().setAutoCommit(false);//throwing error Cannot invoke a
            // rollback operation when the AutoCommit mode is set to "true"

            String fn = "Tester";
            String ln = "Testy";

            // verify that Xiu Mai was originally in database
            try (ResultSet rs = stc
                    .executeQuery("SELECT * FROM employee WHERE FirstName = 'Tester' AND LastName = 'Testy'")) {
                assertTrue(rs.next());
                assertEquals(fn, rs.getString("FirstName"));
                assertEquals(ln, rs.getString("LastName"));
                assertFalse(rs.next());
            ...
            // checks if information was added to login table
            try (ResultSet rs = stc
                    .executeQuery("SELECT * FROM login WHERE FirstName = 'Tester' AND LastName = 'Testy'")) {
                // assertTrue(rs.next());
                // assertNotSame(fn, rs.getString("FirstName"));
                // assertNotSame(ln, rs.getString("LastName"));
                assertFalse(rs.next());
    ...
    void testViewEmployees() throws SQLException {
        ...
        try (Statement stc = d3.sqda.getConnection().createStatement()) {
            ...
            try (ResultSet rs = stc
                    .executeQuery("SELECT * FROM employee WHERE FirstName = 'Xiu Mai' AND LastName = 'Dang'")) {
                assertTrue(rs.next());
            ...
            try (ResultSet rs = stc
                    .executeQuery("SELECT * FROM employee WHERE FirstName = 'Emma' AND LastName = 'Anderson'")) {
                assertTrue(rs.next());
    ...
    void testLogHours() throws SQLException {
        ...
        try (Statement stc = d5.sqda.getConnection().createStatement()) {
            ...
            try (ResultSet rs = stc.executeQuery("SELECT * FROM WorkHours WHERE employeeId = 9")) {
                assertTrue(rs.next());
                assertEquals(id, rs.getInt("9"));
                assertEquals(PayPeriod, rs.getString("2/26/2021 - 3/12/2021"));
                assertEquals(hrs, rs.getInt("8"));
                assertFalse(rs.next());
            }
    ...
    void testRequestLeave() throws SQLException{
        ...
        try (Statement stc = d6.sqda.getConnection().createStatement()) {
            ...
            try (ResultSet rs = stc.executeQuery("SELECT * FROM leave WHERE employeeId = 1")) {
                ...
                assertEquals(anLeave, rs.getInt("23"));
                assertEquals(compLeave, rs.getInt("15"));
                assertEquals(sickLeave, rs.getInt("-5"));
                assertFalse(rs.next());
            }
